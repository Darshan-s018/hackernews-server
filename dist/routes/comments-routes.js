...
const comments_types_1 = require("../controllers/comments/comments-types");
exports.commentsRoutes = new hono_1.Hono();
exports.commentsRoutes.get("/on/:postId", async (c) => {
    const postId = c.req.param("postId");
    const page = Number(c.req.query("page") || 1);
    const pageSize = Number(c.req.query("pageSize") || 10);
    const result = await (0, comments_controller_1.getComments)({ postId, page, pageSize });
...
exports.commentsRoutes.post("/on/:postId", token_middleware_1.tokenMiddleware, async (c) => {
    ...
    const result = await (0, comments_controller_1.createComment)({ userId, postId, text });
...
exports.commentsRoutes.delete("/:commentId", token_middleware_1.tokenMiddleware, async (c) => {
    ...
    try {
        ...
        if (e === comments_types_1.CommentError.NOT_FOUND)
            return c.json({ message: "Comment not found" }, 404);
        if (e === comments_types_1.CommentError.UNAUTHORIZED)
...
exports.commentsRoutes.patch("/:commentId", token_middleware_1.tokenMiddleware, async (c) => {
    ...
    const { text } = await c.req.json();
    try {
        const result = await (0, comments_controller_1.updateComment)({ userId, commentId, text });
        return c.json({ data: result }, 200);
    }
    catch (e) {
        if (e === comments_types_1.CommentError.NOT_FOUND)
